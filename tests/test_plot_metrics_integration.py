...
class TestPlotMetricsIntegration:
    ...
    def test_plot_metrics_generates_all_plot_types(self, simulated_metrics_dir, tmp_path):
        ...
        company_rows = load_csv_rows(
            simulated_metrics_dir / f"company_metrics_{run_id}.csv",
            usecols=None,
            skip_fields={"agent_id"},
        )
        household_rows = load_csv_rows(
            simulated_metrics_dir / f"household_metrics_{run_id}.csv",
            usecols={"time_step", "agent_id"},
        )
        state_rows = load_csv_rows(
            simulated_metrics_dir / f"state_metrics_{run_id}.csv", skip_fields={"agent_id"}
        )
...
class TestPlotMetricsDataExtraction:
    ...
    def test_aggregate_company_metrics(self, tmp_path):
        ...
        rows = load_csv_rows(
            csv_path, usecols=["time_step", "sight_balance", "production_capacity", "rd_investment"]
        )

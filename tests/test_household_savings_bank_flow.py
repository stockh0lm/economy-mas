...
def test_household_child_costs_only_during_growth_phase() -> None:
    ...
    assert remaining <= max(0.0, 300.0 - CONFIG_MODEL.child_rearing_cost)
...
def test_household_growth_phase_triggers_from_bank_savings() -> None:
    ...
    bank.deposit_savings(household, CONFIG_MODEL.savings_growth_trigger + 100.0)
    ...
    assert math.isclose(
        ...
        CONFIG_MODEL.savings_growth_trigger + 100.0,
        rel_tol=1e-6,
    )

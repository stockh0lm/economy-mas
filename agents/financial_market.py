# financial_market.py
from typing import Protocol, TypeVar, Dict, List, Optional, Union, TypeAlias
...
class FinancialMarket(BaseAgent):
    ...
    def trade_assets(
        ...
        log(f"FinancialMarket {self.unique_id}: Trade executed for asset {asset} – "
        ...
        for agent in agents:
            if hasattr(agent, "asset_portfolio"):
                ...
                if total_value > self.speculation_limit:
                    log(f"FinancialMarket {self.unique_id}: Agent {agent.unique_id} holds hyper wealth "

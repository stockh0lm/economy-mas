...
from typing import Protocol, TypedDict, TypeAlias, Dict, Optional
...
class SavingsBank(BaseAgent):
    ...
    def deposit_savings(self, agent: HasUniqueID, amount: float) -> float:
        ...
        """
        if amount <= 0:
            log(f"SavingsBank {self.unique_id}: Invalid deposit amount {amount:.2f} for agent {agent.unique_id}.",
                level="WARNING")
            return 0.0
        ...
        if new_total > self.max_savings_per_account:
            ...
            if allowed_amount > 0:
                ...
                log(f"SavingsBank {self.unique_id}: Agent {agent_id} deposited {allowed_amount:.2f} "
                    f"(max reached; {rejected_amount:.2f} rejected).",
                    level="INFO")
                return allowed_amount
            else:
                log(f"SavingsBank {self.unique_id}: Agent {agent_id} deposit of {amount:.2f} rejected "
            ...
            log(f"SavingsBank {self.unique_id}: Agent {agent_id} deposited {amount:.2f}. "
    ...
    def withdraw_savings(self, agent: HasUniqueID, amount: float) -> float:
        ...
        """
        if amount <= 0:
            log(f"SavingsBank {self.unique_id}: Invalid withdrawal amount {amount:.2f} for agent {agent.unique_id}.",
                level="WARNING")
            return 0.0
        ...
        if current_savings >= amount:
            ...
            log(f"SavingsBank {self.unique_id}: Agent {agent_id} withdrew {amount:.2f}. "
                f"New balance: {self.savings_accounts[agent_id]:.2f}.",
                level="INFO")
            return amount
        else:
            # Insufficient funds
            log(f"SavingsBank {self.unique_id}: Withdrawal of {amount:.2f} for Agent {agent_id} failed. "
    ...
    def allocate_credit(self, borrower: BorrowerAgent, amount: float) -> float:
        ...
        """
        if amount <= 0:
            log(f"SavingsBank {self.unique_id}: Invalid credit request amount {amount:.2f} from {borrower.unique_id}.",
                level="WARNING")
            return 0.0
        ...
        if self.liquidity >= amount:
            ...
            log(f"SavingsBank {self.unique_id}: Insufficient liquidity to allocate credit of {amount:.2f}. "
    ...
    def repayment(self, borrower: HasUniqueID, amount: float) -> float:
        ...
        """
        if amount <= 0:
            log(f"SavingsBank {self.unique_id}: Invalid repayment amount {amount:.2f} from {borrower.unique_id}.",
                level="WARNING")
            return 0.0
        ...
        if outstanding_amount == 0:
            log(f"SavingsBank {self.unique_id}: No outstanding loan for borrower {borrower_id}.",
                level="WARNING")
            return 0.0
        ...
        log(f"SavingsBank {self.unique_id}: Borrower {borrower_id} repaid {repaid_amount:.2f}. "
    ...
    def enforce_savings_limit(self, agent: HasUniqueID) -> float:
        ...
        if current_savings > self.max_savings_per_account:
            ...
            log(f"SavingsBank {self.unique_id}: Enforced savings limit for Agent {agent_id}. "
                f"Excess of {excess_amount:.2f} removed.",
                level="INFO")
            return excess_amount
        else:
            log(f"SavingsBank {self.unique_id}: Agent {agent_id} is within the savings limit.",
                level="DEBUG")
            return 0.0
    ...
    def step(self, current_step: int) -> None:
        ...
        log(f"SavingsBank {self.unique_id}: Maturity matching and clearing operations not implemented yet.",
            level="DEBUG")
